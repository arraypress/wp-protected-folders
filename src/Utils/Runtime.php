...
namespace ArrayPress\ProtectedFolders\Utils;
 ...
 *
 * That is not merely untidy. The transient that records when a folder was
 * last checked, and the filters that decide how a file is delivered, would
 * otherwise be shared by two plugins that know nothing about each other —
 * so one plugin's answer about its own folder would be cached as the other's.
 *
 ...
 * prefix ("MyPlugin\ArrayPress\ProtectedFolders\Utils"), unique per plugin
...
final class Runtime {
	 ...
	 */
	private const LIBRARY = 'protected_folders';

	/**
	 * The per-build prefix.
	 *
	 * "protected_folders" for a plain Composer install — development, or a single
	 * consumer that does not use Strauss — and "{prefix}-protected-folders" for a
	 * prefixed build.
	 *
	 * @return string
	 */
